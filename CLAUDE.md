...
│   └── migrations/               `20260909115445_init` baseline — real migration history (see §13)
└── src/
...
**Changed 2026-09-09 in `wood-vintage` — this diverges from the inherited `unique-dressup` practice.**
`prisma/migrations/` now has a real baseline (`20260909115445_init`), generated by resetting the
then-empty/seed-only `wood_vintage` dev DB and running `prisma migrate dev --name init`. Going
forward, **schema changes go through `npm run prisma:migrate` (`prisma migrate dev --name <desc>`),
not `npm run prisma:push`.** `db push` is no longer the intended workflow for this repo — it still
works (Prisma doesn't forbid it) but using it now would silently drift the DB out of sync with the
migration history, defeating the point. See `../documentation/docs/decisions/` for the rationale
(adopted ahead of the country/pricing schema work in MASTER-PROMPT Phase 1, since retrofitting
migration history gets more expensive the more schema exists on top of it).

For production: still take a `mysqldump` before applying a migration that touches existing data,
and review the generated `migration.sql` before running `prisma migrate deploy` — migration files
are reviewable text, which is the entire point of this change over `db push`.
...
**~~1. Order totals are computed from client-supplied prices.~~ FIXED 2026-09-09 (wood-vintage only).**
`OrderService.createOrder` computed `subtotal` from `data.items[].price` — a value sent by the browser; a crafted request could create an order for any amount. Confirmed exploitable before the fix (tested: ordered a ₹399 product with a spoofed `price: 1`, was charged ₹399 after; would have been charged ₹1 before). Fixed by adding an `effectivePrice()` helper that re-derives price server-side from `productMap`/`variantMap` (same precedence cart.controller.ts already uses: `variant.price ?? product.salePrice ?? product.basePrice`) — `data.items[].price` is no longer read for any money calculation, only accepted for backwards compatibility with the existing request shape. Not backported to `unique-dressup`.
...
**~~12. No migration history.~~ FIXED 2026-09-09 (wood-vintage only).** `prisma/migrations/` now has a real baseline (`20260909115445_init`); schema changes go through `prisma migrate dev`, not `db push`. See §13 "Migration strategy." Not backported to `unique-dressup`, which still uses `db push` as documented.
